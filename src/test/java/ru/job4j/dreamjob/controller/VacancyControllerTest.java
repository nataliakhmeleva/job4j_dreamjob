...
public class VacancyControllerTest {
    ...
    @Test
    public void whenFindVacancyByIdOnPageWithVacanciesThenGetPageWithThisVacancy() {
        var vacancy1 = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var vacancy2 = new Vacancy(2, "test2", "desc2", now(), false, 3, 4);
        var expectedVacancies = List.of(vacancy1, vacancy2);
    ...
    public void whenUpdateVacancyWithFileThenRedirectToVacanciesPage() throws Exception {
        ...
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        var vacancyUpdateArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        when(vacancyService.update(vacancyUpdateArgumentCaptor.capture(), fileDtoArgumentCaptor.capture())).thenReturn(true);

        var model = new ConcurrentModel();
        vacancy.setTitle("new test1");
        var view = vacancyController.update(vacancy, testFile, model);
        var actualVacancy = vacancyUpdateArgumentCaptor.getValue();
        var actualFileDto = fileDtoArgumentCaptor.getValue();
